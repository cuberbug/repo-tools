...
* **Простая настройка окружения:** Скрипт `start` автоматически подготовит рабочее окружение для Python-приложений.
...
1.  **Клонировать** репозиторий:
    ...
    cd repo-tools
    ```
2.  **Запустить скрипт настройки** `start`. Он автоматически создаст виртуальное окружение и установит все необходимые зависимости (включая `rich` и `questionary`):
    ```bash
    ./start
    ```
